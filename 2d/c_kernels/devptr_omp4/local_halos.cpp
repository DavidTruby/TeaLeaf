...
#pragma omp target teams distribute parallel for if(is_offload)
  for (int c = 0; c < (y-2*halo_depth)*depth; ++c) {
      int jj = (c / depth) + halo_depth; int kk = c % depth;
            int base = jj*x;
...
#pragma omp target teams distribute parallel for \
    if(is_offload)
  for (int c = 0; c < depth*(x-2*halo_depth); ++c) {
    int jj = c / (x-2*halo_depth); int kk = c % (x-2*halo_depth)+halo_depth;
            int base = kk;
